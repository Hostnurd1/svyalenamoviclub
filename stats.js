/* =================================================
   MovieClub — stats.js v2.4
   -------------------------------------------------
   Строит краткую статистику:
     • общее количество, средние оценки
     • топ-N фильмов
     • совпавшие оценки
     • тренд «просмотрено / месяц»
   ================================================= */
...
const getWatchTrends = movies => {
  ...
  movies.forEach(m => {
    if (m.status === "Просмотрен" && m.date) {
      const month = (m.date instanceof Date ? m.date.toISOString() : m.date).slice(0,7);
